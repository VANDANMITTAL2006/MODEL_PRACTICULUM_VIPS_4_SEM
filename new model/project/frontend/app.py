"""
Streamlit Frontend — AI Personalized Learning System
...
/* ═══════════════════════════════════════════
   ...
   Matches sidebar: #0f0c29 → #302b63 → #24243e
   ═══════════════════════════════════════════ */
...
def api_predict(payload):
    try:
        ...
        return None, "⚠️ Cannot connect to backend. Is it running? (`uvicorn backend.main:app --reload`)"
...
def api_recommend(payload):
    try:
        ...
        return None, "⚠️ Cannot connect to backend."
    except Exception as e:
        return None, str(e)
...
def badge_for_type(learning_type):
    if learning_type == "Fast Learner":
        return '<span class="badge badge-success">⚡ Fast Learner</span>'
    elif learning_type == "Struggling Learner":
        return '<span class="badge badge-danger">⚠️ Struggling Learner</span>'
    else:
        return '<span class="badge badge-warning">🔄 Low Engagement</span>'
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        ["🏠 Dashboard", "🔮 Predict", "📚 Recommendations", "✏️ Update Quiz", "👤 Profile"],
    ...
    st.markdown("---")
    st.markdown(f"""<div class="streak-box">🔥 Learning Streak<br>{st.session_state.streak} day(s)</div>""", unsafe_allow_html=True)
    st.markdown("---")

    health = api_health()
    if health and health.get("status") == "healthy":
        st.markdown('<span class="badge badge-success">● Backend Online</span>', unsafe_allow_html=True)
    else:
        st.markdown('<span class="badge badge-danger">● Backend Offline</span>', unsafe_allow_html=True)
...
if page == "🏠 Dashboard":
    ...
    st.caption(f"Student ID: **{st.session_state.student_id}** · Adaptive Learning Engine v1.0")
    st.markdown("---")
    ...
    if pred:
        ...
        with col3:
            ...
            for w in wareas:
                st.markdown(f'<span class="badge badge-danger">⚠️ {w}</span>', unsafe_allow_html=True)
        ...
        with col_a:
            ...
            if hist:
                ...
                st.info("📈 No quiz history yet. Go to **Update Quiz** to add entries.")
        ...
        if pred.get("recommended_topics"):
            st.markdown('<div class="section-header">📚 Quick Topic Recommendations</div>', unsafe_allow_html=True)
            pills = " ".join([f'<span class="topic-pill">📌 {t}</span>' for t in pred["recommended_topics"]])
            st.markdown(pills, unsafe_allow_html=True)
    else:
        st.info("👈  Go to **Predict** to analyse a student and populate the dashboard.")
        # Show a teaser
        with st.expander("🎯 What this system does", expanded=True):
            st.markdown("""
| Feature | Description |
|---------|-------------|
| 🔮 **Performance Prediction** | XGBoost model predicts final score |
| 🧠 **Student Segmentation** | KMeans clusters learners into 3 types |
| 📚 **Hybrid Recommendations** | Content-based + Collaborative filtering |
| 🔄 **Real-time Adaptation** | Updates after every quiz attempt |
| 📊 **Rich Analytics** | Gauge charts, line graphs, metric cards |
""")
...
<div class="metric-card">
  ...
  <div style="color:#818cf8;font-size:1.1rem">⚡ {result['engagement_score']:.1f} &nbsp;|&nbsp; 📐 {result['consistency_score']:.1f}</div>
</div>
""", unsafe_allow_html=True)
            with c3:
                st.markdown('<div class="metric-card"><h3>Weak Areas</h3>', unsafe_allow_html=True)
                for w in result["weak_areas"]:
                    st.markdown(f'<span class="badge badge-danger">⚠️ {w}</span><br>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)

            st.markdown("---")
            st.markdown('<div class="section-header">📚 Recommended Topics</div>', unsafe_allow_html=True)
            pills = " ".join([f'<span class="topic-pill">📌 {t}</span>' for t in result["recommended_topics"]])
            st.markdown(pills, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
    ...
    with st.form("recommend_form"):
        ...
        go_rec = st.form_submit_button("🎯 Get Recommendations", use_container_width=True)
    ...
    if go_rec:
        ...
        if err:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.markdown('<div class="section-header">📘 Content-Based Topics</div>', unsafe_allow_html=True)
                for i, t in enumerate(result.get("content_based_topics", []), 1):
                    st.markdown(f"**{i}.** {t}")
            with c2:
                st.markdown('<div class="section-header">👥 Collaborative Topics</div>', unsafe_allow_html=True)
                for i, t in enumerate(result.get("collaborative_topics", []), 1):
                    st.markdown(f"**{i}.** {t}")

            st.markdown("---")
            st.markdown('<div class="section-header">⭐ Top Hybrid Picks</div>', unsafe_allow_html=True)
            pills = " ".join([f'<span class="topic-pill">🏆 {t}</span>' for t in result.get("recommended_topics", [])])
            st.markdown(pills, unsafe_allow_html=True)
    ...
    # Show last result if exists
        ...
        pills = " ".join([f'<span class="topic-pill">📌 {t}</span>' for t in st.session_state.last_recommend.get("recommended_topics", [])])
        st.markdown(pills, unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════
#  PAGE: UPDATE QUIZ
# ══════════════════════════════════════════════════════════
elif page == "✏️ Update Quiz":
    st.markdown('<h1 class="page-title">Update After Quiz</h1>', unsafe_allow_html=True)
    st.caption("Record a new quiz result — the AI will recalibrate your profile in real time.")
    st.markdown("---")
    ...
    with st.form("quiz_update_form"):
        ...
        with c3:
            st.markdown("##### ℹ️ Difficulty Adjustment")
    ...
    if update_btn:
        ...
        with st.spinner("⚡ Adapting learning profile …"):
...
<div class="metric-card">
  <h3>Engagement / Consistency</h3>
  <div style="color:#818cf8">⚡ {result['engagement_score']:.1f} &nbsp;|&nbsp; 📐 {result['consistency_score']:.1f}</div>
</div>
""", unsafe_allow_html=True)

            st.markdown("---")
            st.markdown('<div class="section-header">📚 Updated Recommendations</div>', unsafe_allow_html=True)
            pills = " ".join([f'<span class="topic-pill">📌 {t}</span>' for t in result.get("updated_recommendations", [])])
            st.markdown(pills, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        fetch_btn = st.button("📥 Load Profile", use_container_width=True)

    if fetch_btn or pid:
        with st.spinner(f"Loading profile for **{pid}** …"):
            profile, err = api_profile(pid)

        if err:
            st.error(err)
        elif profile:
            st.success(f"✅ Loaded profile: **{profile['student_id']}**")
            st.markdown("---")
...
<div class="metric-card">
  ...
  <table style="color:#c0c0e0;width:100%;font-size:0.9rem">
    <tr><td>🆔 ID</td><td><b>{profile.get('student_id')}</b></td></tr>
    <tr><td>📅 Age</td><td>{profile.get('age')}</td></tr>
    <tr><td>⚧ Gender</td><td>{profile.get('gender')}</td></tr>
    <tr><td>📖 Learning Style</td><td>{profile.get('learning_style')}</td></tr>
    <tr><td>💪 Strength</td><td>{profile.get('subject_strength')}</td></tr>
    <tr><td>⚠️ Weakness</td><td>{profile.get('subject_weakness')}</td></tr>
    <tr><td>🌐 Internet</td><td>{"Yes" if profile.get('internet_access') else "No"}</td></tr>
    <tr><td>👨‍👩‍👧 Parental Support</td><td>{profile.get('parental_support')}</td></tr>
    <tr><td>😓 Stress Level</td><td>{profile.get('stress_level')}</td></tr>
    <tr><td>🔥 Streak</td><td>{profile.get('streak', 1)} day(s)</td></tr>
  </table>
</div>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
            ...
            if profile.get("weak_areas"):
                st.markdown('<div class="section-header">⚠️ Weak Areas</div>', unsafe_allow_html=True)
