...
from api.schemas.contracts import FeedbackEventRequest, PredictRequest, QuizUpdateRequest, RecommendRequest, SCHEMA_VERSION
...
def make_feature_vector(req_dict: Dict, feat_cols: list):
    ...
    frame = engineer_features(handle_missing_values(frame))
...
def recommend_content(req: RecommendRequest, background_tasks: BackgroundTasks):
    ...
    _queue_shown_events(background_tasks, req.student_id or "anonymous", session_id, result.get("recommended_topics", []))
